const ledStates = {}
const chart = new Chart(document.getElementById('tempHumidityChart').getContext('2d'), {
  type: 'line',
  data: {
    labels: [],
    datasets: [
...
function toggleLED(ledNumber) {
  ...
  publishMessage('g231220017/control', payload)
}
...
function handleHumidity() {
  ...
  if (humidity >= 30 && humidity < 60) {
    humidityLevel = 'Tingkat Kelembaban: Kering/Aman'
  } else if (humidity >= 60 && humidity < 70) {
    humidityLevel = 'Tingkat Kelembaban: Mulai banyak uap air/Normal'
  } else if (humidity >= 70) {
    humidityLevel = 'Tingkat Kelembaban: Terdapat banyak uap air'
  }
